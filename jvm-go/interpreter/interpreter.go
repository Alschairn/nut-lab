...
import (
	...
	"jvm-go/instruction/comparisons"
	"jvm-go/instruction/constants"
	"jvm-go/instruction/control"
	"jvm-go/instruction/conversions"
	"jvm-go/instruction/extended"
	"jvm-go/instruction/loads"
	"jvm-go/instruction/math"
	"jvm-go/instruction/stack"
	"jvm-go/instruction/stores"
	"jvm-go/runtimedata"
)
...
func loop(thread *runtimedata.Thread, bytecode []byte) {
	...
	for {
		...
		if pc == 17 && frame.LocalVars().GetInt(2) <= 101{
			fmt.Println(frame.LocalVars())
		}
		if frame.LocalVars().GetInt(2) > 101 {
			panic("fuck the code")
		}
		inst.Execute(frame)
	}
